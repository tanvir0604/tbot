...
import { setTakeProfit } from "@/lib/binance";

export default function TakeProfitForm({symbol, itemType} : {symbol:string, itemType: string}) {
    ...
    const onSubmit = async (values: z.infer<typeof CreateTPSLSchema>) => {
        const { amount, percentage } = values;    
        try {
            await setTakeProfit({
                symbol: symbol,
                amount: amount,
            ...
            alert('Take profit order placed successfully');
        } catch (error) {
            alert('Failed to place take profit order');
        }
    };
    ...
    return (
        <div className="flex items-center mt-4">
            <Label className="mr-10">Take.P:</Label>
            <Form {...form}>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1">
                                <FormControl>
                                    <Input type="text" placeholder="Amount" value={field.value ?? ''} onChange={field.onChange} />
                                </FormControl>
